...
2. Baixa as páginas públicas por cidade da **Sympla** e da **Eventbrite** (plataformas de ingressos) e extrai o JSON embutido nelas. Para Goiânia, coleta também três fontes oficiais locais: **Goiânia Pulsa** (agenda de turismo, cobre Centro Cultural Oscar Niemeyer, Teatro Goiânia, feiras e congressos), **Centro de Convenções de Goiânia** (ccgo.com.br, agenda oficial do espaço) e **Shopping Cerrado** (eventos infantis e gratuitos). Para Brasília, coleta o **Centro de Convenções Ulysses Guimarães** (agenda oficial com data e hora). Para São Paulo, o **Theatro Municipal** (agenda oficial: ópera, concertos e dança). Para Pirenópolis, a agenda oficial de turismo **pirenopolis.tur.br** (festivais, romarias e programação do Teatro de Pirenópolis). O catálogo completo de fontes sondadas, com o motivo das que ficaram de fora, está em `dados/fontes_catalogo.md`.
3. Coleta também os **links avulsos** de `dados/eventos_avulsos.json`: bilheterias sem listagem pública por cidade (ex.: **BaladAPP**) não permitem varredura, mas a página individual de cada evento é legível. Basta acrescentar a URL do evento nesse arquivo (uma linha) que o robô extrai título, datas, local e cidade e passa a acompanhar. Se a página não tiver título, data e cidade confiáveis, o link é ignorado com aviso no log, nada é inventado.
